...
/// <summary>
/// Separate visualizer component for area selection
/// </summary>
public class AreaSelectionVisualizer : MonoBehaviour
{
    [Header("Visual Settings")]
    public Material selectionMaterial;
    public Color miningColor = new Color(1f, 0f, 0f, 0.3f);
    public Color buildingColor = new Color(0f, 1f, 0f, 0.3f);
    public Color validColor = new Color(0f, 0f, 1f, 0.5f);
    public Color invalidColor = new Color(0.5f, 0.5f, 0.5f, 0.5f);
    
    private GameObject selectionBox;
    private readonly List<GameObject> blockGizmos = new List<GameObject>();
    private readonly List<GameObject> validationGizmos = new List<GameObject>();
    private AreaSelectionManager.SelectionMode currentMode;
    ...
    public void UpdateSelectionBox(Vector3 start, Vector3 end, AreaSelectionManager.SelectionMode mode)
    {
        if (selectionBox == null)
        {
            ...
            DestroyImmediate(selectionBox.GetComponent<Collider>());
    ...
    public void UpdateVisualization(List<Vector3> selected, List<Vector3> valid, 
                                   List<Vector3> invalid, List<Vector3> optimized)
    {
        ClearGizmos();
        ...
        {
            CreateBlockGizmo(block, validColor);
        }
        
        foreach (var block in invalid)
        {
            CreateBlockGizmo(block, invalidColor);
        }
    }
    
    private void CreateBlockGizmo(Vector3 position, Color color)
    {
        GameObject gizmo = GameObject.CreatePrimitive(PrimitiveType.Cube);
        gizmo.name = $"BlockGizmo_{position}";
        gizmo.transform.position = position;
        gizmo.transform.localScale = Vector3.one * 1.1f;
    ...
    {
        foreach (var gizmo in blockGizmos)
        {
            if (gizmo != null)
        ...
        foreach (var gizmo in validationGizmos)
        {
            if (gizmo != null)
                DestroyImmediate(gizmo);
        }
        validationGizmos.Clear();
    }
